...
export const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ isMobile = false }) => {
  ...
  const languages = [
    { code: 'vi', label: t('navigation.langVi'), flagIcon: (props: {className?: string}) => <VietnamFlag {...props} /> },
    { code: 'en', label: t('navigation.langEn'), flagIcon: (props: {className?: string}) => <USFlag {...props} /> },
  ];
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className="flex h-11 items-center gap-2.5 pl-1 pr-3 rounded-full border border-custom-border hover:border-accent bg-secondary-bg/60 hover:bg-secondary-bg transition-all cursor-pointer group focus:outline-none"
        aria-haspopup="true"
        aria-expanded={isOpen}
        aria-label="Select language"
      >
        <div className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center bg-primary-bg shadow-sm shrink-0 border border-custom-border/50">
          {currentLang.flagIcon({ className: "w-full h-full object-cover scale-150" })}
        </div>
        <ChevronDown size={14} className={`text-secondary-text group-hover:text-primary-text transition-transform duration-300 ${isOpen ? 'rotate-180 text-accent' : ''}`} />
      </button>
